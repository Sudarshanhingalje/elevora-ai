...
export default function Login({ initialMode }) {
  ...
  function handleSignup(event) {
    ...
    submit("/api/auth/register", { 
      ...
      agreedToTerms: true
    }, "OTP sent. Check Mailpit/email inbox.", "otp");
  }
  ...
  return (
    <main className="flex h-screen min-w-[1180px] overflow-hidden bg-[#09090b] text-white">
      ...
      <aside className="relative flex w-[520px] shrink-0 flex-col justify-center overflow-hidden border-r border-white/5 bg-[#121318] p-16">
        <Link className="mb-16 font-['Space_Grotesk'] text-3xl font-black text-white flex items-center gap-2" to="/">
        ...
        <p className="mt-16 text-xs text-slate-500">
          HTTPS enforced · Let&apos;s Encrypt SSL · OWASP aligned
        </p>
      </aside>
      ...
      >
        <div className="w-full max-w-[440px] rounded-2xl border border-white/5 bg-[#121318] p-8 md:p-10 shadow-2xl">
          ...
          <button 
            ...
            type="button"
          >
            <span className="grid size-4 place-items-center rounded-full bg-blue-500 text-[9px] font-black text-white">G</span> 
            Google Workspaces
          </button>
          ...
          <div className="mt-6 text-center text-xs text-slate-400">
            {mode === "login" ? (
              <div className="flex flex-col gap-2">
                <button className="font-semibold text-[#ff4d5a] hover:underline" onClick={() => setMode("forgot")}>
                  Forgot password? Reset
                </button>
                <p className="text-slate-500 mt-1">
                  New here? <button className="font-semibold text-[#ff4d5a] hover:underline" onClick={() => setMode("signup")}>Create account</button>
                </p>
              </div>
            ) : (
              <button className="font-semibold text-[#ff4d5a] hover:underline" onClick={() => setMode("login")}>
      ...
      <Modal isOpen={showPrivacyModal} onClose={() => setShowPrivacyModal(false)} title="Privacy Policy">
        ...
        <p>Elevora AI ("we", "us", "our") is dedicated to protecting your privacy and ensuring the security of your data. This Privacy Policy details the types of information we collect and how we utilize it in provisioning our AI and SaaS automation systems.</p>
        
        <h4 className="font-bold text-white mt-4">1. Information We Collect</h4>
        <p>We collect personal identifiers such as full name, email address, phone number, and tenant metadata during user registration to facilitate secure access control and multi-tenant workspace isolation.</p>
        
        <h4 className="font-bold text-white mt-4">2. Usage and n8n Automations</h4>
        <p>Your workspace data, prompt instructions, and social media media paths (including Facebook, Instagram, and ComfyUI outputs) are processed exclusively to execute requested workflow triggers. We do not sell or lease your personal information to third parties.</p>
        
        <h4 className="font-bold text-white mt-4">3. Security and Storage</h4>
        <p>All passwords are encrypted utilizing bcrypt hashing algorithms. User sessions are secured with JSON Web Tokens (JWT) transmitted via HTTP-Only cookies to protect against Cross-Site Scripting (XSS) and Cross-Site Request Forgery (CSRF).</p>
        
        <h4 className="font-bold text-white mt-4">4. Compliance and Rights</h4>
        <p>We comply with standard global regulations. Users retain full rights to request deletion or modification of their account details, tenant slugs, and generated image paths at any time via the User Settings portal.</p>
      </Modal>

      {/* Terms & Conditions Modal */}
      <Modal isOpen={showTermsModal} onClose={() => setShowTermsModal(false)} title="Terms & Conditions">
        <p><strong>Effective Date: June 5, 2026</strong></p>
        <p>By registering an account and deploying AI services on Elevora AI, you agree to comply with and be bound by the following Terms & Conditions. Please read them carefully.</p>
        
        <h4 className="font-bold text-white mt-4">1. Account Security and Authentication</h4>
        <p>Users are solely responsible for safeguarding their login credentials and OTP verification codes. You agree to notify Elevora AI immediately of any unauthorized usage of your multi-tenant workspace credentials.</p>
        
        <h4 className="font-bold text-white mt-4">2. Permitted Use of AI Services</h4>
        <p>You agree not to exploit, reverse-engineer, or overload the platform's API endpoints, ComfyUI nodes, Ollama chat models, or Qdrant vector databases. Abuse of system resources will result in immediate suspension of the tenant space.</p>
        
        <h4 className="font-bold text-white mt-4">3. Downstream API Disclaimers</h4>
        <p>Elevora AI provides integrations with Meta (Facebook/Instagram), WordPress, and payment gateways. We are not liable for rate limits, token expirations, or service disruptions originating from external social media or OAuth platforms.</p>
        
        <h4 className="font-bold text-white mt-4">4. Limitation of Liability</h4>
        <p>SaaS automation features are provided on an "as is" and "as available" basis. Elevora AI makes no warranties regarding the accuracy, completeness, or uninterrupted availability of the deployment workflows.</p>
      </Modal>
    </main>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="w-full max-w-2xl rounded-2xl border border-white/10 bg-[#121318] p-6 shadow-2xl relative flex flex-col max-h-[85vh]">
        <h3 className="font-['Space_Grotesk'] text-xl font-bold text-white mb-4 border-b border-white/5 pb-2">{title}</h3>
...
function AuthForm({ 
  ...
  const placeholders = { name: "John Doe", email: "you@company.com", phone: "9876543210", password: "Password", otp: "123456", newPassword: "Choose new password" };
        ...
        <label className="block" key={field}>
          ...
          <span className="relative block">
            ...
            {field.toLowerCase().includes("password") ? (
              ...
              >
                {visiblePasswords[field] ? <EyeOff size={14} aria-hidden="true" /> : <Eye size={14} aria-hidden="true" />}
              </button>
            ) : null}
          </span>
        </label>
